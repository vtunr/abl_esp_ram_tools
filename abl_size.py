...
def load_map_data(map_file, folder_filter):
    memory_config = load_memory_config(map_file)
    sections  = load_sections(map_file, folder_filter)
...
def create_only_filter(filter):
    if(not filter):
        return ""
    filter_regex = "(?="
    for f in filter:
        filter_regex = filter_regex+"%s|"%f
    filter_regex = filter_regex[:-1]+")"
    #print(filter_regex)
    return filter_regex

def create_exclude_filter(filter):
    if(not filter):
        return ""
    filter_regex = "(?!"
    for f in filter:
...
def create_all_filter(exclude_filter, only_filter):
    return create_exclude_filter(exclude_filter)+create_only_filter(only_filter)

def load_sections(map_file, filter):
    ...
    for line in map_file:
        ...
        RE_SOURCE_LINE = r"\s*(?P<sym_name>\S*).* +0x(?P<address>[\da-f]+) +0x(?P<size>[\da-f]+) (?P<archive>"+filter+r".+\.a)\((?P<object_file>.+\.ob?j?)\)"
        #RE_SOURCE_LINE = r"\s*(?P<sym_name>\S*).* +0x(?P<address>[\da-f]+) +0x(?P<size>[\da-f]+) (?P<archive>.+\.a)\((?P<object_file>.+\.ob?j?)\)"
        #print(RE_SOURCE_LINE)
        m = re.match(RE_SOURCE_LINE, line, re.M)
        if not m:
            # cmake build system links some object files directly, not part of any archive
            RE_SOURCE_LINE = r"\s*(?P<sym_name>\S*).* +0x(?P<address>[\da-f]+) +0x(?P<size>[\da-f]+) (?P<object_file>"+filter+r".+\.ob?j?)"
...
def main():
    ...
    parser.add_argument( 
        '-ef', '--exclude_filter', help='Returns results that exclude what matches this filter', nargs='+')
    parser.add_argument(
        '-of', '--only_filter', help='Returns result that only match this filter', nargs='+'
    )
    parser.add_argument('--limit',type=int, default=-1, help='Hide result with size <=')
    args = parser.parse_args()

    all_filter = create_all_filter(args.exclude_filter, args.only_filter)
    memory_config, sections = load_map_data(args.map_file, all_filter)
    print_summary(memory_config, sections)
    if args.archives:
        print("Per-archive contributions to ELF file:")
        print_detailed_sizes(sections, "archive", "Archive File", args.sort, args.limit)
    if args.files:
        print("Per-file contributions to ELF file:")
        print_detailed_sizes(sections, "file", "Object File", args.sort, args.limit)
...
def print_detailed_sizes(sections, key, header, val_to_sort, limit):
    ...
    headings = (header,
                ...
                "& rodata",
                "Total")
    print("%24s %10s %6s %6s %6s %10s %8s %7s" % headings)
    result = {}
    for k in sizes:
        v = sizes[k]
        result[k] = {}
        ...
        result[k]["dram"] = result[k]["data"] +result[k]["bss"]
        result[k]["bss"] = v.get(".dram0.bss", 0)
    ...
    for k,v in sorted(s, key=return_total_size, reverse=True):
        if ":" in k:  # print subheadings for key of format archive:file
            sh,k = k.split(":")
        if(v[val_to_sort.lower()] <= limit):
            continue
        print("%24s %10d %6d %6d %6d %10d %8d %7d" % (k[:24],
                                                  v["data"],
                                                  v["bss"],
                                                  v["dram"],
                                                  v["iram"],
        ...
        for i in total:
            total[i] = v[i]+total[i]
    print("%24s %10d %6d %6d %6d %10d %8d %7d" % ("Total",
                                                  total["data"],
                                                  total["bss"],
                                                  total["dram"],
                                                  total["iram"],
